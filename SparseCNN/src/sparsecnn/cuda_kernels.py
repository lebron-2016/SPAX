import torch
from sparsecnn.cuda import sparse_conv_bias_wrapper_masked, sparse_deconv_bias_wrapper_masked
from sparsecnn.cuda import deltacnn_activate_truncate, deltacnn_prepare_diff_mask_wrapper
from sparsecnn.cuda import sparse_add_tensors_wrapper, sparse_add_to_dense_tensor_wrapper, sparse_upsample_wrapper, sparse_concatenate_wrapper, sparse_mul_add_wrapper
from sparsecnn.cuda import sparse_pooling_wrapper_masked
from sparsecnn.cuda import deltacnn_init_performance_metrics, deltacnn_reset_performance_metrics, deltacnn_retrieve_metrics
...
def sparse_concatenate(a, b, out=None):
    a, mask_a, mask_backup = a ####################
    b, mask_b, mask_backup = b ####################
    if out is None:
    ...
    return out, mask_out, mask_backup ####################


class SPPerformanceMetrics:
...
class SPPerformanceMetricsManager:
    _initialized = False
    ...
    def get_metrics(cls, n_frames=1):
        ...
        metrics = SPPerformanceMetrics(tiles, inputs, mode, flops, memtransfer, histogram, n_frames=n_frames)
        return metrics
