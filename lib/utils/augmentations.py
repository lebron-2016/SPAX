...
class CropFixArea(object):
    ...
    def __init__(self, opt):
        self.opt = opt
        self.sample_area = (opt.sample_area, opt.sample_area) # (512, 512)
...
class Resize(object):
    ...
    def __call__(self, image, image_fg, boxes, classes, ids, pre_boxes, pre_ids):
        ...
        for i in range(image.shape[3]):
            resized_image = cv2.resize(image[:,:,:,i], dsize=(int(height*self.scale), int(width*self.scale))) #
            resized_images.append(resized_image) 
            resized_image_fg = cv2.resize(image_fg[:,:,:,i], dsize=(int(height*self.scale), int(width*self.scale))) #
            resized_image_fgs.append(resized_image_fg) 
        resized_images = np.stack(resized_images, axis=3)
        resized_image_fgs = np.stack(resized_image_fgs, axis=3)
        
        boxes = boxes * self.scale
        for i in range(self.opt.seqLen - 1):
            pre_boxes[i + 1] = pre_boxes[i + 1] * self.scale
    
        # image = cv2.resize(image, (self.size,
        #                          self.size))
        
        return resized_images, resized_image_fgs, boxes, classes, ids, pre_boxes, pre_ids
