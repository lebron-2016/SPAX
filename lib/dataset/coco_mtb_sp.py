...
class COCO(data.Dataset):
    ...
    def __init__(self, opt, split):
        ...
        if split == 'train':
            ...
            self.annot_path = os.path.join(
                self.img_dir0, 'instances_SatMTB_{}_ship.json').format(split) # 'instances_SatMTB_{}_3cate.json'
    ...
    def __getitem__(self, index):
        ...
        seq_num = self.seqLen
        imIdex = int(file_name.split('.')[0].split('/')[-1])    # 1
        ...
        img_for_motion = np.zeros([im0.shape[0], im0.shape[1], 1, 20])
        ...
        for ii in range(seq_num):
            # 0, 1, ..., seq_num - 1
            imIndexNew = '%06d' % max(imIdex - seq_num + ii + 1, 1)
        ...
        if int(imIdex) < 20:
            xx = 1
            ret = {'now_idx': int(imIdex)-1}
        else:
            xx = int(imIdex) - 20 + 1
            ret = {'now_idx': -1}
        for ii in range(20):
            imIndexNew_for_motion = '%06d' % xx
            imName_for_motion = imf+imIndexNew_for_motion+imtype
            im_for_motion = cv2.imread(self.img_dir + imName_for_motion)
            im_for_motion = cv2.cvtColor(im_for_motion, cv2.COLOR_BGR2GRAY)
            img_for_motion[:,:,0,ii] = im_for_motion
            xx = xx + 1
        ############# for motion mask #############
        ...
        ############# for motion mask #############
        inp_for_motion = img_for_motion.transpose(3, 2, 0, 1).astype(np.float32)
        height, width = img.shape[0] - img.shape[0] % 32, img.shape[1] - img.shape[1] % 32
        #####################
        if int(height) == 2144:
            height = 1856
        if int(width) == 2144:
            width = 1856
        #####################
        inp_for_motion = inp_for_motion[:, :, 0:height, 0:width]
        ret['motion_list'] = inp_for_motion
        ############# for motion mask #############
