...
from sparsecnn.sparse_layers import SPModule, SPConv2d, SPBatchNorm2d, SPActivation

class DeConvGRUCell(SPModule):
    def __init__(self, input_dim, hidden_dim, kernel_size, bias=True, activation=F.tanh):
        ...
        self.conv_z = SPConv2d(in_channels=self.input_dim + self.hidden_dim,
                              out_channels=self.hidden_dim * 2,
                              kernel_size=self.kernel_size,
                              padding=self.padding,
                              bias=self.bias)
        
        self.conv_r = SPConv2d(in_channels=self.input_dim + self.hidden_dim,
                              out_channels=self.hidden_dim,
                              kernel_size=self.kernel_size,
                              padding=self.padding,
                              bias=self.bias)

        self.conv_h1 = SPConv2d(in_channels=self.input_dim,
                              out_channels=self.hidden_dim,
                              kernel_size=self.kernel_size,
                              padding=self.padding,
                              bias=self.bias)

        self.conv_h2 = SPConv2d(in_channels=self.hidden_dim,
                              out_channels=self.hidden_dim,
                              kernel_size=self.kernel_size,
                              padding=self.padding,
                              bias=self.bias)
    ...
    def forward(self, input, h_prev):
        _, _, H, W = input.shape
        combined = torch.cat((input, h_prev), dim=1).contiguous(memory_format=torch.channels_last)  # concatenate along channel axis
        ...
        h_cur = (z1 * h_ + z2 * h_prev).contiguous(memory_format=torch.channels_last)
