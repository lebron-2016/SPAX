...
class DLASeg(nn.Module):
    ...
    def forward(self, img_input, training=True, vid=None, idx=0):
        # x : B, N, C, H, W
        # ..., -3, -2, -1, 0
        ...
        for i in range(N):
            ...
            # mask prop
            if i == 0:
                mask_out = self.mask_layer(p0).unsqueeze(1)
            else:
...
def dla_fpn_net_iter_update_for_satmtb(heads, head_conv=128):
