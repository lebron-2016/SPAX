...
class DLASeg(nn.Module):
    def __init__(self, heads, head_conv, out_channel=0):
        self.inplanes = 256
        self.heads = heads
        self.prev_feat = defaultdict(list) # None
        self.prev_feat[0] = None
        self.prev_feat[1] = None
        self.prev_feat[2] = None
        self.prev_feat[3] = None
        ...
        for head in self.heads:
            ...
            if head_conv > 0:
                fc = nn.Sequential(
                    ...
                    nn.ReLU(inplace=True),
                    nn.Conv2d(head_conv, head_conv, kernel_size=3, stride=1, padding=1, bias=True),
                    nn.ReLU(inplace=True),
        ...
        self.trans_layer = nn.Conv2d(32, 16, kernel_size=3, stride=1, padding=1, bias=True)
        self.trans_layer_gru = nn.Conv2d(32, 32, kernel_size=3, stride=1, padding=1, bias=True)
    ...
    def train_feat(self, x, gru, i):
        ...
        p3 = self.smooth_layer1(self.deconv_layer1(p4) + c3)
        p2 = self.smooth_layer2(self.deconv_layer2(p3) + c2)
        p1 = self.smooth_layer3(self.deconv_layer3(p2) + c1)
        p1 = self.trans_layer_gru(p1 + gru[1])
        p0 = self.smooth_layer4(self.deconv_layer4(p1) + c0)

        return p0, p1, p2, p3

    def track_feat(self, x, gru, i, N, img_id_use=None):
        B, _, H, W = x.shape

        if self.prev_state != False and i < N - 1 and self.prev_feat[img_id_use%4] != None:
           return self.prev_feat[img_id_use%4][i + 1][0], self.prev_feat[img_id_use%4][i + 1][1], self.prev_feat[img_id_use%4][i + 1][2], self.prev_feat[img_id_use%4][i + 1][3]
        else:
            # static down
            c0, c1, c2, c3, c4 = self.backbone(x, gru[0])   # 16, 32, 64, 128
            p4 = c4
    ...
    def forward(self, img_input, training=True, vid=None, idx=0, img_id_use=1):
        # x : B, N, C, H, W
        # ..., -3, -2, -1, 0
        ...
        for i in range(N):
            ...
            if i == 0:
                ...
                c1_cross = c1 + self.trans_layer(p1_pre)
            ...
            x = img_input[:, i, :]
            if training:
                p0, p1, p2, p3 = self.train_feat(x, gru_set, i)
                temp_feat.append([p0, p1, p2, p3])    # N, 4
            else:
                p0, p1, p2, p3 = self.track_feat(x, gru_set, i, N, img_id_use)
                temp_feat.append([p0, p1, p2, p3])    # N, 4
        ...
        # only for track
        if not training:
            self.prev_feat[img_id_use % 4] = temp_feat # interval: 4
...
def dla_fpn_net_iter_update_for_satmtb_light(heads, head_conv=128):
