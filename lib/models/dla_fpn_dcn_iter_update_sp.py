...
class DLASeg(SPModule):
    ...
    def train_feat(self, x, gru, i, p1_pre=None, p2_pre=None, mask=None):
        ...
        # static down
        c1, c2, c3 = self.backbone(x, gru[1])
        # static up
        p3 = c3  # B, 256, 64, 64
    ...
    def track_feat(self, x, gru, i, N, p1_pre=None, p2_pre=None, mask=None, vid=None):
        ...
        if self.prev_state != False and i < N - 1:
            ...
            # static down
            c0, c1, c2, c3, c4 = self.backbone(x, gru[0])
    ...
    def forward(self, img_input, img_input_for_motion, now_idx, training=True, vid=None, idx=0):
        ...
        expanded_mask = (expanded_mask > 0).to(torch.float32)  # 二值 mask 
        ################### mask ####################
        ...
        for i in range(N):
            ...
            # 这里目前使用当前的mask，如果前面的mask不对应，则是0
            if type(p0) == tuple:
        ...
        if not training:
            ...
            self.prev_pred_mask = (self.prev_pred_mask >= 0.2).float() # my: 0.2
        ...
        return ret, self.mask_backup[1024]
...
def dla_fpn_net_iter_update_sp(heads, head_conv=128):
