...
def cal_flops(model, batch_dict):
    macs, params = profile(model, inputs=(batch_dict, False, 'flag'))
    macs2 = macs / 1e9
    return macs2

def process(model, image, vid=None, idx=0):
    with torch.no_grad():
        output_all = model(image, training=False, vid=vid, idx=idx)[-1]
        output = output_all[1]
...
def test(opt, split, modelPath, show_flag, results_name):
    ...
    for ind, (img_id, pre_processed_images) in enumerate(data_loader):
        ...
        # cur image detection
        dets_all, cur_dis = process(model, image, vid=file_folder_cur+'_'+str(im_count), idx=ind+1)
        ...
        if file_folder_buffer[0] != file_folder_pre and len(file_folder_buffer[0]) != 0:
            ...
            plane_tracker = BYTETracker(opt)   # 2
            ship_tracker = BYTETracker(opt)    # 3
